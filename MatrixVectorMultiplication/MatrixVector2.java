	...
	public static void Mapper()throws IOException {
		...
		while((line=br.readLine()) != null) {
			...
			if(currentValue == null )
				keyvalue.put(Integer.parseInt(array[0]),array[2]+" "+array[1]);
			else {
				currentValue+=" "+(array[2]+" "+array[1]);
	...
	public static void Reducer()throws IOException {
		...
		while(itr.hasNext()) {
			...
			for(int i=0;i< elements.length; i+=2) {
				...
				int column=Integer.parseInt(elements[i+1]);
				output+=(element*vector.get(column));
			}
